...
import DraftReviewBar from "@/components/industry-news/DraftReviewBar";
...
// Look an article up by slug. Published only, unless `allowDraft` is set — the
// admin reader passes it so an unpublished article can be read before approval.
...
// The article reader itself. The public route renders it as-is; the admin
// reader (/news-preview/[id]) passes `preview` so an article that is still a
// draft — or was rejected — can be read and approved before it goes public.
export default async function ArticleScreen({
  ...
  const article = await getArticle(slug, { allowDraft: preview });
  if (!article) notFound();
  ...
  return (
    <div className="min-h-screen bg-white text-slate-800">
      ...
      {/* Admin-only review bar — never rendered on the public route. */}
      {preview && id && <DraftReviewBar id={id} status={status} />}
