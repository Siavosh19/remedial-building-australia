...
export default async function DashboardIndexPage() {
  ...
  const [leadsReceived, newLeads, recentLeads] = await Promise.all([
    ...
    prisma.quoteRequestDelivery.findMany({
      ...
      take: 5,
      include: {
        request: { select: { suburb: true, postcode: true, urgency: true, work_category: { select: { name: true } } } },
      },
    }),
  ]);

  const leadCards: DashboardLead[] = recentLeads.map((d) => ({
    id: d.id,
    category: d.request.work_category?.name ?? "Building works",
    location: `${d.request.suburb} ${d.request.postcode}`.trim(),
    date: new Date(d.created_at).toLocaleDateString("en-AU", { day: "numeric", month: "short" }),
    urgency: d.request.urgency ? (URGENCY_LABELS[d.request.urgency] ?? null) : null,
    isNew: !d.opened_at,
    interested: !!d.interested_at,
  }));
  ...
  return (
    <div className="space-y-6">
      ...
      {/* ── Recent leads (full width, with Interested buttons) ──────────── */}
        ...
        {leadCards.length === 0 ? (
          <div className="flex flex-col items-center justify-center px-6 py-12 text-center">
            ...
            <p className="mt-1 text-xs text-slate-400">
              Matching quote requests from strata managers and owners will appear here. Tap &ldquo;Interested&rdquo; and we&rsquo;ll refer you straight to the client.
            </p>
            {plan === "basic" && (
              <Link
          ...
          <DashboardLeadCards leads={leadCards} />
        )}
      </div>

      {/* ── Profile checklist — collapsible, below the leads ─────────────── */}
      <ProfileChecklistCollapsible
        items={checklist.map((c) => ({ label: c.label, done: c.done, locked: c.locked }))}
