...
import { LiquidAppliedUnderTileProductSection, LiquidAppliedUnderTileIntroSection } from "./LiquidAppliedUnderTileProductSection";
import PageNav from "@/components/PageNav";

import SiteHeader from "@/components/SiteHeader";
import SeoCrossPromo from "@/components/sections/SeoCrossPromo";
export const metadata: Metadata = {
  title: "Liquid-Applied Membranes — Under-Tile — Balcony Waterproofing — Remedial Building Australia",
  description:
    "Technical product reference for liquid-applied undertile waterproofing membranes for balcony and wet area waterproofing remediation in Australian Class 2 strata buildings — acrylic-based undertile systems.",
};
...
const SIBLING_GROUPS = [
  {
    ...
    tabs: [
      ...
      { label: "LQ undertile", slug: "liquid-applied-membranes-under-tile" },
...
export default function LiquidAppliedUnderTilePage() {
  return (
    <div className="min-h-screen bg-slate-50">
      ...
      <main>
        ...
        <section className="border-b border-slate-200 bg-white px-4 sm:px-8 py-12">
          <div className="mx-auto max-w-7xl">
            <nav className="mb-5 flex flex-wrap items-center gap-2 text-xs font-semibold text-slate-400">
              ...
              <span>/</span>
              <span className="text-sky-950">Liquid-applied membranes — under-tile</span>
            </nav>
            <PageNav />
              ...
              {SIBLING_GROUPS.map((group, gi) => (
                <div key={group.heading} className={`flex shrink-0 flex-col${gi > 0 ? " border-l border-slate-200 ml-1 pl-1" : ""}`}>
                  ...
                  <div className="flex items-end">
                    {group.tabs.map((tab) => {
                      const active = tab.slug === "liquid-applied-membranes-under-tile";
                      return (
                        <Link key={tab.slug} href={`/repair-systems/balcony-waterproofing-failure/${tab.slug}`}
        ...
        <section className="px-4 sm:px-8 py-14">
          <div className="mx-auto max-w-7xl space-y-10">
            <LiquidAppliedUnderTileIntroSection />
            <LiquidAppliedUnderTileProductSection />
          </div>
        </section>
      ...
      <footer className="border-t border-slate-200 bg-slate-100">
        <div className="mx-auto max-w-7xl px-5 pt-10">
          <Link href="/repair-systems/balcony-waterproofing-failure" className="inline-flex rounded-xl border border-sky-200 bg-white px-4 py-2 text-sm font-bold text-sky-950 shadow-sm transition hover:bg-slate-200">← Balcony Waterproofing Failure</Link>
        </div>
