...
import { SaltAffectedRenderProductSection, SaltAffectedRenderIntroSection } from "./SaltAffectedRenderProductSection";

export const metadata: Metadata = {
  title: "Salt-Affected Render Systems — Rising Damp — Remedial Building Australia",
  description: "Technical product reference for salt-affected render systems — Westox Plastalite Rehabilitation Render for masonry and brick walls subject to rising damp on Australian strata and commercial buildings.",
};

const ACTIVE_SLUG = "salt-affected-render-systems";
...
export default function SaltAffectedRenderPage() {
  return (
    <div className="min-h-screen bg-slate-50">
      ...
      <main>
        <section className="border-b border-slate-200 bg-white px-8 py-12"><div className="mx-auto max-w-7xl"><nav className="mb-5 flex flex-wrap items-center gap-2 text-xs font-semibold text-slate-400"><a href="/" className="hover:text-sky-700 transition">Home</a><span>/</span><a href="/repair-systems" className="hover:text-sky-700 transition">Repair Systems</a><span>/</span><a href={BASE_URL} className="hover:text-sky-700 transition">Rising Damp</a><span>/</span><span className="text-sky-950">Salt-affected render systems</span></nav><div className="grid gap-8 lg:grid-cols-[1fr_340px]"><div><p className="text-sm font-bold uppercase tracking-[0.28em] text-red-700">Repair Systems — Rising Damp</p><h1 className="mt-3 text-4xl font-extrabold leading-tight tracking-tight text-sky-950 md:text-5xl">Salt-affected render systems</h1><p className="mt-5 max-w-2xl text-base leading-7 text-slate-600">Technical product reference for salt-affected render systems — Westox Plastalite Rehabilitation Render for masonry and brick walls affected by rising damp and salt contamination.</p></div><div className="grid grid-cols-2 gap-3 self-start rounded-2xl border border-slate-200 bg-slate-50 p-5">{[{ label: "Products listed", value: "1" }, { label: "Brand", value: "Westox" }].map((s) => <div key={s.label} className="rounded-xl border border-slate-100 bg-white p-3 text-center"><div className="text-lg font-extrabold leading-tight text-sky-950">{s.value}</div><div className="mt-0.5 text-[10px] font-semibold uppercase tracking-wider text-slate-400">{s.label}</div></div>)}</div></div></div></section>
        ...
        <section className="px-8 py-14"><div className="mx-auto max-w-7xl space-y-10"><SaltAffectedRenderIntroSection /><SaltAffectedRenderProductSection /></div></section>
