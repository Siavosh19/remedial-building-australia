...
import { CheckCircle, AlertTriangle, BookOpen, ExternalLink, ChevronLeft, ChevronRight, FileText, XCircle } from "lucide-react";

type FilterTag = "High-build" | "Acrylic" | "Facade-membrane" | "Masonry" | "Render" | "Water-based" | "Elastomeric";
...
const PRODUCTS: Product[] = [
  {
    ...
    accentColor: "#64748b",
    name: "Westox WBA Membrane",
    descriptionLine: "High-build acrylic membrane coating for masonry and render facades — confirm current formulation, coverage, and system specifications with Westox technical",
    productType: "High-build acrylic membrane — facade",
    filterTags: ["High-build", "Acrylic", "Facade-membrane", "Masonry", "Render", "Water-based"],
    techChips: [{ label: "High-build", cls: "bg-slate-100 text-slate-700" }, { label: "Acrylic", cls: "bg-slate-100 text-slate-700" }, { label: "Facade", cls: "bg-slate-100 text-slate-700" }],
    systemDescription: "Westox WBA Membrane is a high-build acrylic membrane coating for masonry and render facades. High-build coatings are applied at higher film thickness than standard topcoats, providing increased build on the substrate surface.\n\nConfirm current product technical data sheet, coverage rate, DFT, application method, primer requirements, and system design with Westox technical before specifying. Do not rely on this general reference as a substitute for current Westox WBA Membrane technical documentation.",
    technicalProperties: ["High-build acrylic coating — applied at higher film thickness than standard exterior topcoats", "Confirm DFT, coverage rate, application method, and primer requirements from current Westox TDS", "Confirm substrate suitability and primer specification with Westox technical"],
    limitations: ["Confirm current product formulation and system specifications with Westox technical before specifying", "Confirm primer requirements for the substrate type from Westox technical", "Confirm current Australian product availability with Westox before specifying"],
    procurementSources: [{ name: "Westox — contact for trade supply", url: "https://www.westox.com.au" }],
  },
];

const FILTER_DEFS: { id: FilterTag; label: string }[] = [{ id: "High-build", label: "High-build" }, { id: "Acrylic", label: "Acrylic" }, { id: "Masonry", label: "Masonry" }, { id: "Render", label: "Render" }];
...
export function HighBuildAcrylicIntroSection() {
  return (
    <div className="rounded-2xl border border-slate-200 bg-white p-7 shadow-sm">
      <div className="mb-4 flex items-center gap-2.5"><div className="flex h-8 w-8 items-center justify-center rounded-lg bg-sky-950 text-white"><BookOpen size={15} /></div><h3 className="text-base font-extrabold text-sky-950">High-build acrylic membrane facade coatings</h3></div>
      <p className="text-sm leading-7 text-slate-600">High-build acrylic membrane coatings are applied at higher film thickness than standard exterior topcoats, providing increased build and a more substantial coating layer on masonry and render facades. Confirm current product specifications, DFT requirements, and system design with Westox technical before specifying.</p>
    </div>
  );
}

export function HighBuildAcrylicProductSection() {
  ...
  return (
    <div>
      <div className="mb-5 flex items-start gap-3"><div className="mt-1 h-5 w-1 shrink-0 rounded-full bg-red-700" /><div><h2 className="text-2xl font-extrabold text-sky-950">Product Reference</h2><p className="mt-1 text-sm text-slate-500">1 product — Westox — high-build acrylic membrane facade systems</p></div></div>
