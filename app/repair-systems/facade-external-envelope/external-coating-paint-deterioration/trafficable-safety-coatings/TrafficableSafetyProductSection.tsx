...
type FilterTag = "Trafficable" | "Safety-coating" | "Anti-slip" | "Concrete" | "Masonry" | "Coloured" | "Westox";
...
const PRODUCTS: Product[] = [
  {
    ...
    accentColor: "#64748b",
    name: "Westox Wesdex Safety Colours 15 Litre",
    descriptionLine: "Trafficable safety coating in safety colours — anti-slip properties for concrete and masonry surfaces — confirm current formulation, colour range, and system specifications with Westox technical",
    productType: "Trafficable safety coating — coloured anti-slip — concrete and masonry",
    filterTags: ["Trafficable", "Safety-coating", "Anti-slip", "Concrete", "Masonry", "Coloured", "Westox"],
    techChips: [{ label: "Safety colours", cls: "bg-slate-100 text-slate-700" }, { label: "Anti-slip", cls: "bg-slate-100 text-slate-700" }, { label: "Trafficable", cls: "bg-slate-100 text-slate-700" }],
    systemDescription: "Westox Wesdex Safety Colours is a trafficable safety coating available in safety colours (typically yellow, red, white, blue and other high-visibility colours) for marking and coating concrete and masonry surfaces subject to foot traffic. Safety coatings combine colour marking with anti-slip properties for pedestrian safety applications.\n\nConfirm current product technical data sheet, available colour range, anti-slip aggregate options, coverage rate, and substrate requirements with Westox technical before specifying.",
    technicalProperties: ["Trafficable safety coating — available in safety colours for line marking and hazard identification", "Anti-slip properties — confirm aggregate type and anti-slip classification from current Westox TDS", "Confirm colour range, coverage rate, DFT, and substrate preparation requirements from current Westox TDS"],
    limitations: ["Confirm current product formulation, colour range, and system specifications with Westox technical before specifying", "Confirm substrate preparation and primer requirements with Westox before specifying", "Confirm current Australian product availability with Westox before specifying"],
    procurementSources: [{ name: "Westox — contact for trade supply", url: "https://www.westox.com.au" }],
  },
];

const FILTER_DEFS: { id: FilterTag; label: string }[] = [{ id: "Trafficable", label: "Trafficable" }, { id: "Safety-coating", label: "Safety coating" }, { id: "Anti-slip", label: "Anti-slip" }, { id: "Concrete", label: "Concrete" }];
...
export function TrafficableSafetyIntroSection() {
  return (<div className="rounded-2xl border border-slate-200 bg-white p-7 shadow-sm"><div className="mb-4 flex items-center gap-2.5"><div className="flex h-8 w-8 items-center justify-center rounded-lg bg-sky-950 text-white"><BookOpen size={15} /></div><h3 className="text-base font-extrabold text-sky-950">Trafficable safety coatings</h3></div><p className="text-sm leading-7 text-slate-600">Trafficable safety coatings combine surface protection with anti-slip properties and safety colour marking for pedestrian areas on concrete and masonry. Confirm current Westox Wesdex Safety Colours product specifications, available colour range, anti-slip rating, and substrate requirements with Westox technical before specifying.</p></div>);
}

export function TrafficableSafetyProductSection() {
  ...
  return (
    <div>
      <div className="mb-5 flex items-start gap-3"><div className="mt-1 h-5 w-1 shrink-0 rounded-full bg-red-700" /><div><h2 className="text-2xl font-extrabold text-sky-950">Product Reference</h2><p className="mt-1 text-sm text-slate-500">1 product — Westox — trafficable safety coating systems</p></div></div>
