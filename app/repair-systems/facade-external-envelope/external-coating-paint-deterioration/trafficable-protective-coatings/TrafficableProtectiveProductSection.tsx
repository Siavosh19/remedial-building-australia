...
import { CheckCircle, BookOpen, ExternalLink, ChevronLeft, ChevronRight, FileText, XCircle } from "lucide-react";

type FilterTag = "Trafficable" | "Protective-coating" | "Concrete" | "Masonry" | "Floor-coating" | "Westox";
...
const PRODUCTS: Product[] = [
  {
    ...
    accentColor: "#64748b",
    name: "Westox Wesdex 15 Litre",
    descriptionLine: "Trafficable protective coating for concrete and masonry surfaces — confirm current formulation, coverage, substrate suitability, and system specifications with Westox technical",
    productType: "Trafficable protective coating — concrete and masonry",
    filterTags: ["Trafficable", "Protective-coating", "Concrete", "Masonry", "Floor-coating", "Westox"],
    techChips: [{ label: "Trafficable", cls: "bg-slate-100 text-slate-700" }, { label: "Protective", cls: "bg-slate-100 text-slate-700" }, { label: "Westox", cls: "bg-slate-100 text-slate-700" }],
    systemDescription: "Westox Wesdex is a trafficable protective coating for concrete and masonry surfaces. Trafficable coatings are formulated to withstand pedestrian and/or light vehicle traffic while providing a protective barrier to the substrate.\n\nConfirm current product technical data sheet, coverage rate, substrate suitability, primer requirements, and application conditions with Westox technical before specifying. Do not rely on this general reference as a substitute for current Westox Wesdex technical documentation.",
    technicalProperties: ["Trafficable protective coating — suitable for pedestrian traffic on concrete and masonry", "Confirm substrate requirements, primer specification, and DFT from current Westox Wesdex TDS", "Confirm current application method and conditions from Westox technical"],
    limitations: ["Confirm current product formulation and system specifications with Westox technical before specifying", "Confirm substrate preparation and primer requirements with Westox before specifying", "Confirm current Australian product availability with Westox before specifying"],
    procurementSources: [{ name: "Westox — contact for trade supply", url: "https://www.westox.com.au" }],
  },
];

const FILTER_DEFS: { id: FilterTag; label: string }[] = [{ id: "Trafficable", label: "Trafficable" }, { id: "Protective-coating", label: "Protective coating" }, { id: "Concrete", label: "Concrete" }, { id: "Masonry", label: "Masonry" }];
...
export function TrafficableProtectiveIntroSection() {
  return (<div className="rounded-2xl border border-slate-200 bg-white p-7 shadow-sm"><div className="mb-4 flex items-center gap-2.5"><div className="flex h-8 w-8 items-center justify-center rounded-lg bg-sky-950 text-white"><BookOpen size={15} /></div><h3 className="text-base font-extrabold text-sky-950">Trafficable protective coatings</h3></div><p className="text-sm leading-7 text-slate-600">Trafficable protective coatings are applied to concrete and masonry surfaces that are subject to foot traffic or light vehicle traffic. They provide a protective and durable surface while withstanding the mechanical demands of regular use. Confirm current Westox Wesdex specifications, DFT, substrate requirements, and application conditions with Westox technical before specifying.</p></div>);
}

export function TrafficableProtectiveProductSection() {
  ...
  return (
    <div>
      <div className="mb-5 flex items-start gap-3"><div className="mt-1 h-5 w-1 shrink-0 rounded-full bg-red-700" /><div><h2 className="text-2xl font-extrabold text-sky-950">Product Reference</h2><p className="mt-1 text-sm text-slate-500">1 product — Westox — trafficable protective coating systems</p></div></div>
