...
import { TrafficableProtectiveProductSection, TrafficableProtectiveIntroSection } from "./TrafficableProtectiveProductSection";
import PageNav from "@/components/PageNav";

import SiteHeader from "@/components/SiteHeader";
import SeoCrossPromo from "@/components/sections/SeoCrossPromo";
export const metadata: Metadata = {
  title: "Trafficable Protective Coatings — External Coating — Remedial Building Australia",
  description: "Technical product reference for trafficable protective coatings — Westox Wesdex for concrete and masonry surfaces on Australian strata and commercial buildings.",
};
  ...
  return (
    <div className="min-h-screen bg-slate-50">
      ...
      <main>
        ...
        <section className="px-4 sm:px-8 py-14"><div className="mx-auto max-w-7xl space-y-10"><TrafficableProtectiveIntroSection /><TrafficableProtectiveProductSection /></div></section>
        <section className="border-t border-slate-200 bg-slate-50 px-4 sm:px-8 py-10"><div className="mx-auto max-w-7xl"><div className="rounded-2xl border border-amber-200 bg-amber-50 px-6 py-5"><p className="mb-1 text-[10px] font-bold uppercase tracking-wider text-amber-700">Disclaimer</p><p className="text-xs leading-6 text-amber-900">This page provides general technical information only. Confirm product selection against current TDS and project specification. Do not rely on this reference as a substitute for professional advice.</p></div><div className="mt-8"><a href={BASE_URL} className="inline-flex items-center gap-2 rounded-xl border border-sky-200 bg-white px-4 py-2 text-sm font-bold text-sky-950 shadow-sm hover:bg-slate-200 transition"><ArrowRight size={14} className="rotate-180" /> Back to External Coating &amp; Paint Deterioration</a></div></div></section>
      </main>
      <SeoCrossPromo />
