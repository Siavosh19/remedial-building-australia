...
export const metadata: Metadata = {
  title: "Masonry Cleaning — Chemical Poultice — Brickwork Deterioration — Remedial Building Australia",
  description:
    "Technical product reference for chemical poultice masonry cleaning systems for masonry facade remediation on Australian Class 2 strata buildings — absorbent poultice cleaners for deep stain extraction from porous masonry substrates.",
};

const ACTIVE_SLUG = "masonry-cleaning-chemical-poultice";
...
export default function MasonryPoultPage() {
  return (
    <div className="min-h-screen bg-slate-50">
      ...
      <main>
        <section className="border-b border-slate-200 bg-white px-4 sm:px-8 py-12">
          <div className="mx-auto max-w-7xl">
            <nav className="mb-5 flex flex-wrap items-center gap-2 text-xs font-semibold text-slate-400">
              ...
              <span className="text-sky-950">Masonry cleaning — chemical</span>
            </nav>
            <PageNav />
            <div className="grid gap-8 lg:grid-cols-[1fr_340px]">
              <div>
                ...
                <h1 className="mt-3 text-2xl sm:text-3xl font-extrabold leading-tight tracking-tight text-sky-950 md:text-5xl">
                  Masonry cleaning — chemical poultice
                </h1>
                <p className="mt-5 max-w-2xl text-base leading-7 text-slate-600">
                  Technical product reference for chemical poultice masonry cleaning systems for masonry facade remediation on Australian Class 2 strata buildings. Covers absorbent clay or paper-pulp poultice carriers combined with chemical cleaning agents for deep extraction of embedded stains from porous masonry — used where surface washing cannot remove deeply set organic, biological or mineral stains without damaging the masonry surface. Suitable for acid-sensitive substrates where acid washing is not appropriate.
                </p>
              </div>
              <div className="grid grid-cols-2 gap-3 self-start rounded-2xl border border-slate-200 bg-slate-50 p-5">
                {[
                  { label: "Poultice types", value: "3" },
                  { label: "Risk", value: "Low" },
                  { label: "Type", value: "Clay-based" },
                  { label: "For", value: "Embedded stains" },
                ].map((s) => (
        ...
        <section className="px-4 sm:px-8 py-14">
          <div className="mx-auto max-w-7xl">
            <h2 className="mb-2 text-xl font-extrabold text-sky-950">Masonry chemical poultice cleaning types</h2>
            <p className="mb-8 max-w-2xl text-sm leading-6 text-slate-600">Select the poultice cleaning type below to compare products, technical data, and procurement sources.</p>
            <div className="grid gap-5 sm:grid-cols-3">
              {[
                {
                  href: `${BASE_POULT}/attapulgite-hydrogen-peroxide-poultice`,
                  label: "Organic stains",
                  title: "Attapulgite / H₂O₂ Poultice",
                  tag: "Low risk",
                  description: "Attapulgite clay + H₂O₂ — for organic staining, biological residue, and mould — low chemical risk",
                  suppliers: "Doff · Stonecare · DIY formulation",
                },
                {
                  href: `${BASE_POULT}/kaolin-solvent-poultice`,
                  label: "Oil stains",
                  title: "Kaolin Solvent Poultice",
                  tag: "Medium risk",
                  description: "Kaolin clay + organic solvent — for oil, grease and hydrocarbon staining — solvent handling required",
                  suppliers: "Stonecare · Doff · DIY formulation",
                },
                {
                  href: `${BASE_POULT}/proprietary-masonry-poultice`,
                  label: "Multi-stain",
                  title: "Proprietary Masonry Poultice",
                  tag: "Commercial",
                  description: "Commercially formulated multi-stain poultice cleaners — specialist heritage and restoration products",
                  suppliers: "Doff · Diedrich · Resene",
                },
              ].map((card) => (
                <a key={card.href} href={card.href} className="group block rounded-2xl border border-slate-200 bg-white p-6 transition hover:border-sky-300 hover:shadow-md">
                  <div className="mb-3 flex items-center justify-between">
                    ...
                    <span className="inline-block rounded-md bg-green-50 px-2.5 py-1 text-[10px] font-bold uppercase tracking-wider text-green-700">{card.tag}</span>
                  </div>
                  ...
                  <div className="flex items-center gap-1 text-xs font-bold text-sky-700 transition group-hover:text-red-700">View products <ArrowRight size={11} /></div>
                </a>
              ))}
            </div>
          </div>
        ...
        <section className="border-t border-slate-200 bg-slate-50 px-4 sm:px-8 py-10">
          <div className="mx-auto max-w-7xl">
            <div className="rounded-2xl border border-amber-200 bg-amber-50 px-6 py-5">
              ...
              <p className="text-xs leading-6 text-amber-900">This page provides general technical information only. Chemical poultice selection must be matched to the specific stain type and masonry substrate. Trial application on an inconspicuous area is required before full-scale treatment. OH&S requirements for chemical handling must be complied with.</p>
            </div>
            <div className="mt-8 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
              {[
                ...
                { href: "/defect-library/facade-external-envelope", label: "Defect Library", title: "Masonry staining and surface contamination defects" },
