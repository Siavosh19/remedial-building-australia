...
import { TwoCoatPolymerRenderIntroSection, TwoCoatPolymerRenderProductSection } from "./TwoCoatPolymerRenderProductSection";
import PageNav from "@/components/PageNav";

import SiteHeader from "@/components/SiteHeader";
import SeoCrossPromo from "@/components/sections/SeoCrossPromo";
export const metadata: Metadata = {
  title: "Two-Coat Polymer-Modified Render Systems — Render Cracking — Remedial Building Australia",
  description:
    "Technical product reference for two-coat polymer-modified cementitious render repair systems — Sika MonoTop, Weber and Rockcote systems for scratch coat and finish coat re-render on concrete and masonry facades.",
};
...
export default function TwoCoatPolymerRenderPage() {
  return (
      ...
      <main>
        <section className="border-b border-slate-200 bg-white px-8 py-12">
          <div className="mx-auto max-w-7xl">
            <nav className="mb-5 flex flex-wrap items-center gap-2 text-xs font-semibold text-slate-400">
              ...
              <span className="text-sky-950">Two-coat polymer-modified render</span>
            </nav>
            <PageNav />
            <div className="grid gap-8 lg:grid-cols-[1fr_340px]">
              ...
              <div className="grid grid-cols-2 gap-3 self-start rounded-2xl border border-slate-200 bg-slate-50 p-5">
                {[
                  { label: "System type", value: "2-coat" },
        ...
        <section className="px-8 py-14">
          <div className="mx-auto max-w-7xl space-y-10">
            <TwoCoatPolymerRenderIntroSection />
            <TwoCoatPolymerRenderProductSection />
          </div>
        </section>
        ...
        <section className="border-t border-slate-200 bg-slate-50 px-8 py-10">
          <div className="mx-auto max-w-7xl">
            <div className="rounded-2xl border border-amber-200 bg-amber-50 px-6 py-5">
              ...
              <p className="text-xs leading-6 text-amber-900">This page provides general technical information only. Render system selection, coat thickness, primer and topcoat requirements must be confirmed against substrate type, existing render system, defect type, and the current manufacturer TDS. Confirm compliance with AS 3700 and applicable NCC requirements. Do not rely on this reference as a substitute for professional facade or building consultant advice.</p>
            </div>
            <div className="mt-8 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
              {[
                ...
                { href: "/repair-systems/facade-external-envelope/render-removal-and-installation", label: "Related Defect", title: "Render removal and installation" },
                { href: BASE_URL, label: "Back to defect", title: "Render Cracking &amp; Delamination" },
              ].map((card) => (
                <a key={card.href} href={card.href} className="group block rounded-2xl border border-slate-200 bg-white p-5 transition hover:border-sky-200 hover:shadow-md">
                  <div className="mb-2 text-[10px] font-bold uppercase tracking-wider text-red-700" dangerouslySetInnerHTML={{ __html: card.label }} />
                  <h4 className="text-sm font-extrabold leading-snug text-sky-950" dangerouslySetInnerHTML={{ __html: card.title }} />
