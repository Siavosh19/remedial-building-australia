...
type FilterTag = "Bagging" | "Render-finish" | "Masonry" | "Brick" | "Concrete-block" | "Pre-bagged" | "Hand-applied" | "Westox";
...
const PRODUCTS: Product[] = [
  {
    ...
    accentColor: "#64748b",
    name: "Westox Plastalite Bagging Mix 20kg",
    descriptionLine: "Bagging mix for masonry, brick, and concrete block facades — semi-rough or slurry-fill finish — confirm current formulation, water ratio, coverage, and system specifications with Westox technical",
    productType: "Bagging mix — masonry, brick and concrete block facades",
    filterTags: ["Bagging", "Render-finish", "Masonry", "Brick", "Concrete-block", "Pre-bagged", "Hand-applied", "Westox"],
    techChips: [{ label: "Bagging mix", cls: "bg-slate-100 text-slate-700" }, { label: "Pre-bagged", cls: "bg-slate-100 text-slate-700" }, { label: "Hand-applied", cls: "bg-slate-100 text-slate-700" }],
    systemDescription: "Westox Plastalite Bagging Mix is a pre-bagged bagging mix for masonry, brick, and concrete block facade surfaces. Bagging is a traditional Australian render technique where a wet cementitious slurry is applied over brick or masonry with a hessian sack or similar applicator to fill joints and create a semi-rough or slurry-fill surface texture, which is then overcoated with a paint or texture coating.\n\nBagging is commonly used in remedial facade works to resurface deteriorated masonry or brickwork facades before repainting, providing a consistent substrate for the new coating system.\n\nConfirm current product technical data sheet, water ratio, coverage, application method, curing requirements, and compatible topcoat system with Westox technical before specifying.",
    technicalProperties: [
      "Pre-bagged bagging mix — suitable for masonry, brick, and concrete block facade surfaces",
      "Provides semi-rough or slurry-fill finish suitable for subsequent paint or texture coating application",
      "Confirm water ratio, coverage, application method, and compatible topcoat from current Westox Plastalite Bagging Mix TDS",
    ],
    limitations: [
      "Confirm current product formulation and system specifications with Westox technical before specifying",
      "Bagging does not provide the same performance as a full two-coat render system — not appropriate for severely degraded or structurally compromised masonry",
...
const FILTER_DEFS: { id: FilterTag; label: string }[] = [{ id: "Bagging", label: "Bagging" }, { id: "Render-finish", label: "Render finish" }, { id: "Masonry", label: "Masonry" }, { id: "Brick", label: "Brick" }, { id: "Pre-bagged", label: "Pre-bagged" }];
...
export function BaggingRenderIntroSection() {
  return (<div className="rounded-2xl border border-slate-200 bg-white p-7 shadow-sm"><div className="mb-4 flex items-center gap-2.5"><div className="flex h-8 w-8 items-center justify-center rounded-lg bg-sky-950 text-white"><BookOpen size={15} /></div><h3 className="text-base font-extrabold text-sky-950">Bagging render coats</h3></div><p className="text-sm leading-7 text-slate-600">Bagging is a traditional Australian surface finish technique where a wet cementitious mix is applied over masonry, brick, or concrete block surfaces with a hessian applicator to fill joints and produce a semi-rough or slurry-fill texture. Bagging is used in remedial works to resurface deteriorated masonry facades before repainting. Westox Plastalite Bagging Mix is a pre-bagged product for this application. Confirm current specifications with Westox technical before specifying.</p></div>);
}

export function BaggingRenderProductSection() {
  ...
  return (
    <div>
      <div className="mb-5 flex items-start gap-3"><div className="mt-1 h-5 w-1 shrink-0 rounded-full bg-red-700" /><div><h2 className="text-2xl font-extrabold text-sky-950">Product Reference</h2><p className="mt-1 text-sm text-slate-500">1 product — Westox — bagging mix systems</p></div></div>
