...
// ──────────────────────────────────────────────────────────────────────────────
// PMSpecCard — PAGE-LOCAL fork of ProductSpecCardV2, used ONLY by the
// polymer-modified repair mortars page so the layout can be iterated here without
// affecting the other repair-system pages. Once the design is finalised it can be
// promoted back into the shared ProductSpecCardV2.
//
// The four selection sections (Key properties / Best for / Avoid where / Key
// warnings) are COLLAPSIBLE. The open/closed state is owned by the parent and
// shared across every visible card, so toggling a section opens it on all cards
// at once — the cards stay aligned (CSS subgrid) and read straight across.
// ──────────────────────────────────────────────────────────────────────────────
...
import type { RefCard, BadgeTone } from "../../_components/ProductSpecCardV2";
  ...
  return (
    ...
    >
      ...
      {card.appInfo.length > 0 ? (
        <SectionBox
          id="props" open={open.has("props")} onToggle={onToggle} mt="mt-4"
          boxClass="bg-[#F1F5F9]"
          icon={<Ruler size={15} className="text-slate-500" />} title="Key properties"
        >
          <ul className="space-y-2.5">
            {card.appInfo.map((it, i) => (
      ...
      {card.bestFor.length > 0 ? (
        <SectionBox
          id="best" open={open.has("best")} onToggle={onToggle} mt="mt-2.5"
          boxClass="bg-[#F0F9FF]"
      ...
      {card.avoidWhere.length > 0 ? (
        <SectionBox
          id="avoid" open={open.has("avoid")} onToggle={onToggle} mt="mt-2.5"
          boxClass="bg-[#FFF7ED]"
      ...
      {card.warnings.length > 0 ? (
        <div className="mb-4">
          <SectionBox
            id="warn" open={open.has("warn")} onToggle={onToggle} mt="mt-2.5"
            boxClass="bg-[#FEF2F2]"
            ...
            <div>
              ...
              <div className="overflow-hidden rounded-lg border border-slate-200">
                <table className="w-full text-[12px]">
                  <tbody>
                    {card.advanced.techData.map((r, i) => (
